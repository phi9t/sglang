...
DEFAULT_MLSYS_VENV_ROOT="/repo/.mlsys-venvs"
readonly DEFAULT_MLSYS_VENV_ROOT
...
Env vars:
  ...
  MLSYS_VENV_ROOT        Venv root in container (default: /repo/.mlsys-venvs).
...
require_digest_ref() {
    ...
    # Allow local tag refs (for locally available images). If a digest is
    # provided, validate its format strictly.
    [[ "${ref}" == *@sha256:* ]] || return 0
    local digest
    digest="${ref##*@sha256:}"
    [[ "${digest}" =~ ^[a-f0-9]{64}$ ]] || {
...
docker run "${docker_args[@]}" \
    ...
    "${ENV_INPUT}" \
    --venv-root "${MLSYS_VENV_ROOT:-${DEFAULT_MLSYS_VENV_ROOT}}" \
    "$@"
