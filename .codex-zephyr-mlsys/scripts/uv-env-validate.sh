...
cleanup() {
    rm -f "${ENV_JSON_FILE}" "${GPU_TEST_TEMP_FILES[@]:-}"
}
trap cleanup EXIT
...
venv_field() {
    local index="$1"
    local field="$2"
    VENV_JSON="${VENV_JSON}" "${PYTHON_BIN}" - "${index}" "${field}" <<'PY'
import json
import os
import sys

venv = json.loads(os.environ["VENV_JSON"])[int(sys.argv[1])]
print(venv[sys.argv[2]])
PY
}

validation_list() {
    local field="$1"
    VALIDATION_JSON="${VALIDATION_JSON}" "${PYTHON_BIN}" - "${field}" <<'PY'
import json
import os
import sys

values = json.loads(os.environ["VALIDATION_JSON"])[sys.argv[1]]
print(" ".join(values))
PY
}
...
print("true" if json.loads(os.environ["VALIDATION_JSON"])[sys.argv[1]] else "false")
PY
}

validation_gpu_field() {
    local index="$1"
    local field="$2"
    VALIDATION_JSON="${VALIDATION_JSON}" "${PYTHON_BIN}" - "${index}" "${field}" <<'PY'
import json
import os
import sys

gpu_script = json.loads(os.environ["VALIDATION_JSON"])["gpu_functional"][int(sys.argv[1])]
value = gpu_script[sys.argv[2]]
...
find_venv_for_package() {
    ...
    get_venv_dir "$(venv_field 0 "name")"
}
...
VALIDATION_SPACK_PROVENANCE="$(validation_list "spack_provenance")"
VALIDATION_UV_PROVENANCE="$(validation_list "uv_provenance")"
VALIDATION_NO_NVIDIA_PIP="$(validation_bool "no_nvidia_pip")"
VALIDATION_HARD_FAIL_ON="$(validation_list "hard_fail_on")"
VALIDATION_SOFT_FAIL_ON="$(validation_list "soft_fail_on")"
VALIDATION_GPU_SCRIPT_COUNT="$(
    VALIDATION_JSON="${VALIDATION_JSON}" "${PYTHON_BIN}" - <<'PY'
import json
import os

print(len(json.loads(os.environ["VALIDATION_JSON"])["gpu_functional"]))
PY
)"
...
for pkg in ${VALIDATION_SPACK_PROVENANCE}; do
    ...
    first_venv="$(get_venv_dir "$(venv_field 0 "name")")"
...
if [[ "${GPU_TESTS}" != "true" ]]; then
    ...
    for i in $(seq 0 $((VALIDATION_GPU_SCRIPT_COUNT - 1))); do
        test_name="$(validation_gpu_field "${i}" "name")"
        test_script="$(validation_gpu_field "${i}" "script")"
        hard_fail_patterns="$(validation_gpu_field "${i}" "hard_fail_patterns")"

        # Find appropriate venv — for multi-venv, match test name to venv name
        target_venv=""
        for j in $(seq 0 $((VENV_COUNT - 1))); do
            vn="$(venv_field "${j}" "name")"
        ...
        if [[ -z "${target_venv}" ]]; then
            ...
            target_venv="$(get_venv_dir "$(venv_field 0 "name")")"
        fi
        ...
        GPU_TEST_TEMP_FILES+=("${tmp_script}")
