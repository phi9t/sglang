...
#   ./container/verify_uv_layering.sh --skip-datasets          # skip T8.7 datasets check
#   ./container/verify_uv_layering.sh --datasets-timeout-sec 180
#
# Exit codes:
#   0 - All tests passed
...
WITH_VLLM=false
SKIP_DATASETS=false
DEFAULT_TIMEOUT_SEC="${ZEPHYR_VERIFY_TIMEOUT_SEC:-900}"
DATASETS_TIMEOUT_SEC="${ZEPHYR_VERIFY_DATASETS_TIMEOUT_SEC:-180}"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --skip-datasets)
            ...
            shift
            ;;
        --datasets-timeout-sec)
            DATASETS_TIMEOUT_SEC="${2:-}"
            if [[ -z "${DATASETS_TIMEOUT_SEC}" ]]; then
                echo "Missing value for --datasets-timeout-sec" >&2
                exit 2
            fi
            shift 2
            ;;
        --help|-h)
            echo "Usage: $0 [IMAGE] [--no-gpu] [--with-vllm] [--skip-datasets] [--datasets-timeout-sec N]"
            echo ""
            ...
            echo "  --skip-datasets  Skip T8.7 datasets install (non-blocking)"
            echo "  --datasets-timeout-sec N  Timeout for T8.7 datasets stage (default: 180)"
            exit 0
            ;;
        -*)
...
readonly GPU
readonly WITH_VLLM
readonly SKIP_DATASETS
readonly DEFAULT_TIMEOUT_SEC
readonly DATASETS_TIMEOUT_SEC
...
run_no_gpu() {
    ...
    verify_run_no_gpu "${IMAGE}" "${cmd}" "${timeout_sec}" 2>&1
}
...
run_with_gpu() {
    local cmd="$1"
    local timeout_sec="${2:-${DEFAULT_TIMEOUT_SEC}}"
    verify_run_with_gpu "${IMAGE}" "${cmd}" "${timeout_sec}" 2>&1
}
...
pkgs = ['torch', 'torchvision', 'torchaudio', 'jax', 'numpy', 'scipy', 'triton']
for pkg in pkgs:
    mod_name = pkg.replace('-', '_')
    try:
        mod = importlib.import_module(mod_name)
    except Exception as e:
        print(f'NOT_SPACK:{pkg}:IMPORT_ERROR:{e}')
        continue

    f = getattr(mod, '__file__', '') or ''
    if '/opt/spack_store/view' in f:
        print(f'SPACK_OK:{pkg}')
    else:
        print(f'NOT_SPACK:{pkg}:{f}')
PY
")" || true

for pkg in ${SPACK_PKGS}; do
    if echo "${result}" | grep -q "^SPACK_OK:${pkg}$"; then
        pass "${pkg} from Spack view"
    else
        detail="$(echo "${result}" | grep "^NOT_SPACK:${pkg}:" | head -1 || true)"
        [[ -n "${detail}" ]] || detail="no output for ${pkg}"
        fail "${pkg} from Spack view" "got '${detail}'"
    fi
done
...
pkgs = ['transformers', 'tokenizers', 'accelerate']
for pkg in pkgs:
    mod_name = pkg.replace('-', '_')
    try:
        mod = importlib.import_module(mod_name)
    except Exception as e:
        print(f'UNKNOWN:{pkg}:IMPORT_ERROR:{e}')
        continue
    ...
    if '/tmp/test-venv' in f:
        ...
        print(f'SPACK_VIEW:{pkg}:{f}')
    else:
        print(f'UNKNOWN:{pkg}:{f}')
PY
")" || true

for pkg in ${UV_PKGS}; do
    if echo "${result}" | grep -q "^UV_OK:${pkg}$"; then
        pass "${pkg} from UV venv"
    else
        detail="$(echo "${result}" | grep -E "^(SPACK_VIEW|UNKNOWN):${pkg}:" | head -1 || true)"
        [[ -n "${detail}" ]] || detail="no output for ${pkg}"
        fail "${pkg} from UV venv" "got '${detail}'"
    fi
done
...
\"
        ")" || true
        if echo "${result}" | grep -q "^HF_OK$"; then
            pass "HuggingFace transformers tokenizer load"
        else
            fail "HuggingFace transformers tokenizer load" "output tail: $(echo "${result}" | tail -5)"
        fi
    fi
fi
...
if [[ "${SKIP_DATASETS}" == "true" ]]; then
    echo "  WARN: datasets install skipped (--skip-datasets)"
else
    DATASETS_SETUP='
set -euo pipefail
export PATH="/usr/local:/usr/local/bin:${PATH}"
...
'
    ...
    result="$(run_no_gpu "${DATASETS_SETUP}"$'\n'"echo DATASETS_INSTALL_OK" "${DATASETS_TIMEOUT_SEC}")" || true
    if echo "${result}" | grep -q "DATASETS_INSTALL_OK"; then
        ...
        echo "  WARN: datasets install failed/timeout (non-blocking)"
        echo "        timeout_sec=${DATASETS_TIMEOUT_SEC}"
